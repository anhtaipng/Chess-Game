            ...
            if(state.pieceSrcHashCode){
                ...
                try{
                    if(BoardHelper.checkMoveValidity(pos1, pos2, pieces_copy)){
                        ...
                        return {...state,pieces: pieces_copy ,pieceSrcHashCode: undefined,currentTurn: newTurn};
                    }
                }
                catch (e) {
                    console.log(e);
...
const GameBoard = () => {
    ...
    const handleSquareClick = (posHashCode) => {
        if(!state.pieceSrcHashCode){
            dispatch({type:GAME_ACTION.CHOOSE_PIECE,payload:{
                    srcHashCode:posHashCode
                }})
            console.log("Piece chosen:" + posHashCode);
        } else{
            dispatch({type:GAME_ACTION.CHOOSE_DEST,payload:{
                ...
                <div className={styles.boardGrid}>
                    {
                        hashCodeArr.map((hashCode)=> {
                            ...
                            return <Square tabIndex={hashCode.toString()} key={hashCode.toString()} posHashCode={hashCode}
                                    dispatch={dispatch} isLightSquare={isLightSquare} image={image} onClick={(hashCode) => handleSquareClick(hashCode)}
                            />
                            }
                        )
                    }
